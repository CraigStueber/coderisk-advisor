...
Endpoints:
  ...
  POST /api/upload     — Normalize a file upload before analysis
...
Session handling:
  ...
  NOTE: X-Session-ID is client-supplied and not server-validated against
  an authenticated identity. Any client with a valid UUID can resume that
  session. Acceptable for a demo; requires a session ownership store before
  production use with sensitive code submissions.
"""
...
def findings_event(
    ...
    payload: dict = {"vuln": vuln_findings, "behavioral": behavioral_findings}
...
async def stream_graph_response(
    ...
    """
    Run the LangGraph graph and yield SSE events.

    Event sequence per turn:
      agent_status: running   — as each node starts
      agent_status: complete  — as each node finishes
      findings                — emitted once after Skeptic completes
      token                   — streamed from Synthesizer output
    ...
    # Guard against duplicate running events for the same node
    ...
    try:
        async for event in graph.astream_events(state, config=config, version="v2"):
            ...
            # Synthesizer: emit tokens then complete status
            ...
            # All other nodes
                ...
                if isinstance(output, dict):
                    if name == "vuln_scanner":
                        count = len(output.get("vuln_findings", []))
                        detail = f"{count} finding{'s' if count != 1 else ''}"

                    elif name == "behavioral_risk":
                        count = len(output.get("behavioral_findings", []))
                        detail = f"{count} behavioral risk{'s' if count != 1 else ''}"

                    elif name == "skeptic":
                        assessment = output.get("skeptic_assessment")
                        disputed = (
                            len(assessment.get("disputed_finding_ids", []))
                            if isinstance(assessment, dict)
                            else 0
                        )
                        detail = f"{disputed} disputed" if disputed else "no disputes"

                        # Findings are finalized after the Skeptic pass.
                        # Read from thread state so disputed flags are current.
                        ...
                        count = len(output.get("remediation_items", []))
...
async def analyze(request: Request, body: AnalyzeRequest):
    """
    Main analysis endpoint. Returns an SSE stream.

    Session lifecycle:
      First request:       no X-Session-ID header  -> new session_id issued
      Subsequent requests: X-Session-ID present     -> existing thread resumed
    """
    ...
    if body.code and body.code.strip():
        if body.flagged_as_ai_generated:
            submission_type = SubmissionType.AI_GENERATED
        elif body.filename:
            submission_type = SubmissionType.FILE
        else:
            submission_type = SubmissionType.SNIPPET
...
async def upload_file(request: Request):
    """
    Accept a source file upload and return normalized code content.
    The frontend passes the result to /api/analyze.
    """
    ...
    ext = next((e for e in ACCEPTED_EXTENSIONS if file.filename.endswith(e)), None)
    if ext is None:
        raise HTTPException(
            status_code=400,
    ...
    if len(lines) > MAX_FILE_LINES:
        raise HTTPException(
            status_code=400,
            detail=f"File too large: {len(lines)} lines. Maximum is {MAX_FILE_LINES} lines.",
        )
